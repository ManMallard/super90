...
static inline uint32_t ticksGetMillis(void) { return xTaskGetTickCount(); }
...
menuStatus_t menuKeyEntry(uiEvent_t *ev, bool isFirstRun)
{
    switch (s_mode)
    {
        case MODE_PASS:  return menu_pass (ev, isFirstRun);
        case MODE_HEX:   return menu_hex  (ev, isFirstRun);
        case MODE_LABEL: return menu_label(ev, isFirstRun);
    }
    return MENU_STATUS_SUCCESS;
}
